...
Spawn all explorers in a single message, one Agent tool call each:

- `subagent_type`: `Explore` (the read-only search agent)
- `model`: `sonnet`
...
Call the Agent tool once for a subagent that explores and explains in one pass:
...
Once all explorers return, call the Agent tool once to synthesize their findings into one coherent explanation:
...
After the explanation is complete, spawn one architectural critic per model in the critic roster (`opus`, `fable`, `sonnet`), all in a single message.

For each critic:
- `subagent_type`: `Explore` (the read-only search agent)
- `model`: one model from the roster. Only Anthropic models are available here, so three critics is the full spread. Give each a distinct angle in its prompt; with fewer model families, the framing is what makes the critics independent rather than redundant.
