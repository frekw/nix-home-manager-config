...
Before spawning investigators, enumerate the MCP servers this session can reach. MCP tools are named `mcp__<server>__<tool>`. Some are loaded up front; others are deferred and appear by name only in system reminders, so call `ToolSearch` (keyword queries, or `select:<name>`) to pull in the schemas for anything that looks relevant. Read each server's instructions block when one is present, since it usually names the server's evidence domain outright.
...
Subagent config (each), one Agent tool call per investigator:
- `subagent_type`: `general-purpose`
- `model`: `sonnet`

Use `general-purpose`, not `Explore`. Investigators need MCP access, and each one has to call `ToolSearch` itself to load its server's schemas before it can query anything. Tell every investigator to write nothing. That's a posture, not a sandbox.
...
Use `general-purpose` here too. The synthesizer's quality check spot-verifies citations, which can require MCP access.
