...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Invoke the `why` skill with the Skill tool and use its step 2 to pull the PR and commits.
...
6. For a big or wide change, run it through the `arena` skill. Ask several models the same question and merge the answers. Different models catch different real bugs. Only Anthropic models are available here (`opus`, `fable`, `sonnet`), so vary the question's framing per runner as well, not just the model.
